...
- **Авто-MTProxy под белыми списками:** парсер больше не выбрасывает прокси с IP вместо домена (в RU-списке это было 61 из 100), читает формат `host:port:secret`, а не только ссылки `tg://proxy`, и ставит FakeTLS-прокси (`ee`-секрет, подставляют SNI реального домена) первыми — и в очередь проверки, и в выбор до первого ответа. Пул 40 хранимых / 32 проверяемых, добавлен RU-список Chumbayoumba.
