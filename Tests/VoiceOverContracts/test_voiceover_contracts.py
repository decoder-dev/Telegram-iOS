...
class ReleaseGateContractTests(unittest.TestCase):
    """Only the artefacts this fork actually adopted.

    The upstream branch also asserted a pull-request template and a CI workflow.
    Both are repository-wide process choices rather than code, and this fork's
    pull requests are mostly networking work, so imposing a VoiceOver checklist
    on every one of them was not ours to decide. Adopt them and put the
    assertions back.
    """
    ...
    def test_release_process_artifacts_are_present(self) -> None:
        for artifact in (
            ...
            "Tests/VoiceOverContracts/test_voiceover_contracts.py",
        ):
