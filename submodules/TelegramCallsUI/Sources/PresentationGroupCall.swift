...
public final class PresentationGroupCallImpl: PresentationGroupCall {
    ...
    public init(
        ...
        if useIPCContext {
            ...
            screencastIPCContext = ScreencastInProcessIPCContext(basePath: self.accountContext.sharedContext.basePath, isConference: self.isConference, e2eContext: self.e2eContext, proxyServer: self.accountContext.sharedContext.callManager?.resolvedCallProxyServer())
        ...
        if shouldJoin, let callInfo = activeCallInfo {
            ...
            if let current = self.genericCallContext {
                ...
                if self.isStream, self.accountContext.sharedContext.immediateExperimentalUISettings.liveStreamV2 {
                    ...
                    }, isConference: self.isConference, audioIsActiveByDefault: audioIsActiveByDefault, isStream: self.isStream, sharedAudioDevice: self.sharedAudioContext?.audioDevice, encryptionContext: encryptionContext, proxyServer: self.accountContext.sharedContext.callManager?.resolvedCallProxyServer()))
