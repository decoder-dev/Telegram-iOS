...
private final class ExpandItemView: UIView {
    ...
    func updateTheme(theme: PresentationTheme) {
        self.updateStyle(self.style, theme: theme)
    }
...
public final class ReactionContextNode: ASDisplayNode, ASScrollViewDelegate {
    ...
    private var isTapbacksBottomPickerActive: Bool {
        return self.usesExternalExpandButton && self.isExpanded
    }
    ...
    /// has taken it.
    ///
    /// Between the two stops the drag changes the sheet's height, so more of the grid comes into
    /// view as it rises; above the top stop that is damped hard, since there is nowhere left to go.
    /// Below the resting stop the sheet stops shrinking and slides down the screen instead, one to
    ...
    private var hideExpandedTopPanelBeforeExpand: Bool?
    ...
    public init(context: AccountContext, animationCache: AnimationCache, presentationData: PresentationData, style: Style = .legacy, usesTapbacksLayout: Bool = false, items: [ReactionContextItem], selectedItems: Set<AnyHashable>, title: String? = nil, reactionsLocked: Bool, alwaysAllowPremiumReactions: Bool, allPresetReactionsAreAvailable: Bool, getEmojiContent: ((AnimationCache, MultiAnimationRenderer) -> Signal<EmojiPagerContentComponent, NoError>)?, isExpandedUpdated: @escaping (ContainedViewLayoutTransition) -> Void, requestLayout: @escaping (ContainedViewLayoutTransition) -> Void, requestUpdateOverlayWantsToBeBelowKeyboard: @escaping (ContainedViewLayoutTransition) -> Void) {
        ...
        if getEmojiContent != nil && !self.reactionsLocked {
            ...
            if self.usesExternalExpandButton {
                expandItemView.updateStyle(.externalSmile, theme: presentationData.theme)
                // Parent views assigned after super.init below.
            } else {
                expandItemView.updateStyle(.inlineArrow, theme: presentationData.theme)
        ...
        if self.usesExternalExpandButton, let expandItemView = self.expandItemView {
            // Sibling of the glass pill — mirrors iMessage's emoji control under Tapbacks.
            self.view.addSubview(expandItemView)
            self.view.insertSubview(expandItemView.tintView, belowSubview: expandItemView)
            expandItemView.tintView.isHidden = true
        }
                ...
                if let reactionSelectionComponentHost = strongSelf.reactionSelectionComponentHost, let componentView = reactionSelectionComponentHost.view {
                    ...
                    let _ = reactionSelectionComponentHost.update(
                        ...
                        component: AnyComponent(EmojiStatusSelectionComponent(
                            ...
                            // Must agree with the layout pass below, otherwise an emoji-content refresh
                            // pops the Telegram pack panel back into the docked sheet.
                            disableTopPanel: strongSelf.alwaysAllowPremiumReactions || strongSelf.hideExpandedTopPanel || isBottomDocked,
    ...
    private func updateScrolling(transition: ContainedViewLayoutTransition) {
        ...
        for i in 0 ..< self.items.count {
            ...
            if appearBounds.intersects(baseItemFrame) || (self.visibleItemNodes[i] != nil && visibleBounds.intersects(baseItemFrame)) {
                ...
                if !itemNode.isExtracted {
                    ...
                    if !self.usesExternalExpandButton && !self.reactionsLocked && self.getEmojiContent != nil && i == itemLayout.visibleItemCount - 1 {
                    ...
                    if !self.usesExternalExpandButton, !self.reactionsLocked, self.getEmojiContent != nil, i == itemLayout.visibleItemCount - 1, let itemNode = itemNode as? ReactionNode {
        ...
        if let expandItemView = self.expandItemView, !self.usesExternalExpandButton {
    ...
    private func updateLayout(size: CGSize, insets: UIEdgeInsets, anchorRect: CGRect, centerAligned: Bool, isCoveredByInput: Bool, isAnimatingOut: Bool, forceUpdate: Bool = false, transition: ContainedViewLayoutTransition, animateInFromAnchorRect: CGRect?, animateOutToAnchorRect: CGRect?, animateReactionHighlight: Bool = false) {
        if let expandItemView = self.expandItemView {
            switch self.reactionStyle {
            case .glass:
                expandItemView.updateStyle(.externalSmile, theme: self.presentationData.theme)
            case .legacy:
                expandItemView.updateStyle(.inlineArrow, theme: self.presentationData.theme)
            }
        }
        ...
        if !self.reactionsLocked && self.getEmojiContent != nil {
            // Inline expand steals one slot; external (iMessage) smile control sits outside the pill.
            let totalItemSlotCount = self.items.count + (self.usesExternalExpandButton ? 0 : 1)
        ...
        let tapbacksBottomPickerActive = self.isTapbacksBottomPickerActive
        ...
        if self.usesExternalExpandButton {
            // The pill keeps its capsule shape in both states — expanding docks the picker at the
            // bottom rather than growing the pill in place.
        ...
        if (self.isExpanded || (self.reactionSelectionComponentHost != nil && !self.isCollapsing)), let _ = self.getEmojiContent, !self.reactionsLocked {
            ...
            if let emojiContent = self.emojiContent {
                ...
                let pickerGeometry: (height: CGFloat, bottomOverscroll: CGFloat) = tapbacksBottomPickerActive
                    ? self.tapbacksPickerGeometry(size: size, insets: insets)
                    : (height: self.emojiContentHeight, bottomOverscroll: 0.0)
                let pickerContainerHeight = pickerGeometry.height
            ...
            if let componentView = reactionSelectionComponentHost.view {
                ...
                // The external Tapbacks smile fades with the pill in the layout pass above; only the
                // inline arrow needs the scale-back-in counterpart of its expand animation.
                if let expandItemView = self.expandItemView, !self.usesExternalExpandButton {
                ...
                // The bottom-docked picker leaves the way it arrived — downwards, not up into the screen.
                let dismissOffsetY: CGFloat = componentView.superview === self.view ? componentView.bounds.height : -30.0
                componentView.isUserInteractionEnabled = false
                transition.updatePosition(layer: componentView.layer, position: componentView.layer.position.offsetBy(dx: 0.0, dy: dismissOffsetY))
    ...
    private func updateEmojiContent(_ emojiContent: EmojiPagerContentComponent) {
        ...
        // Keyed off the style rather than the live expanded flag, to match the styling the sheet is
        // built with — in Tapbacks the grid only ever appears docked, and a content refresh arriving
        // while it animates away must not re-impose the capsule metrics on it.
        let contentCustomLayout: EmojiPagerContentComponent.CustomLayout? = self.usesExternalExpandButton ? nil : emojiContentLayout
        ...
        if self.usesExternalExpandButton || self.backgroundNode.backgroundTintMaskContainer.isHidden {
    ...
    public func animateIn(from sourceAnchorRect: CGRect) {
        ...
        if !self.presentationData.reduceMotion && !self.reduceMotion {
            for i in 0 ..< self.items.count {
                ...
                if let itemLayout = self.itemLayout, !self.usesExternalExpandButton, !self.reactionsLocked, self.getEmojiContent != nil, i == itemLayout.visibleItemCount - 1 {
    ...
    override public func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        if self.isTapbacksBottomPickerActive {
            // The picker is docked to the bottom of self.view, outside contentContainer, so the
            // contentContainer branch below would never reach it. Anything outside the picker must
            // fall through to the host (tapping away dismisses).
            if let componentView = self.reactionSelectionComponentHost?.view, componentView.superview === self.view {
                return componentView.hitTest(self.view.convert(point, to: componentView), with: event)
            }
            return nil
        }
    ...
    public func expand() {
        ...
        if self.usesExternalExpandButton {
            ...
            self.hideExpandedTopPanelBeforeExpand = self.hideExpandedTopPanel
    ...
    public func collapse() {
        ...
        self.currentContentHeight = self.usesExternalExpandButton
            ...
            : 46.0
        if let hideExpandedTopPanelBeforeExpand = self.hideExpandedTopPanelBeforeExpand {
