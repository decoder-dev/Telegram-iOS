...
public class ContactsController: ViewController {
    ...
    private func deactivateSearch(animated: Bool) {
        ...
        (self.parent as? TabBarController)?.updateIsTabBarHidden(ForkExtrasHotFlags.hidesTabBar(isPad: UIDevice.current.userInterfaceIdiom == .pad), transition: .animated(duration: 0.5, curve: .spring))
        self.requestLayout(transition: .animated(duration: 0.5, curve: .spring))
    }
